...
Бот-секретарь для Telegram, который автоматически отвечает на сообщения через AI от вашего имени. Персонаж и стиль задаются через промт командой `/setprompt` прямо из Telegram. Поддерживает обычные личные сообщения и **Telegram Business / автоматизацию чатов**. Имитирует живого человека: показывает индикатор набора текста с реалистичной задержкой.

## Возможности

- Отвечает от вашего имени в заданном стиле (промт задаётся через `/setprompt`)
- Поддерживает Telegram Business (автоматизация чатов)
- Имитирует набор текста с реалистичной задержкой (15 симв/сек ±20%)
...
| `OWNER_ID` | — | Telegram ID владельца — только он может управлять промтом и историей |
...
## Upstash Redis (история и промт)

История диалогов и активный промт хранятся в Upstash Redis и переживают рестарты Railway.
...
Все команды доступны только владельцу (`OWNER_ID`). Узнать свой Telegram ID можно через [@userinfobot](https://t.me/userinfobot).

| Команда | Описание |
|---|---|
| `/setprompt <текст>` | Задать промт текстом прямо в команде |
| `/setprompt` | Бот попросит прислать промт следующим сообщением (текст или .md файл) |
| `/getprompt` | Показать текущий активный промт |
| `/clearhistory` | Очистить историю диалогов всех пользователей |

При смене промта история очищается автоматически.
...
Технические настройки (не секреты) хранятся в `config.toml`:

```toml
[bot]
chars_per_second = 15          # скорость имитации набора текста
...
Промт задаётся через `/setprompt` и хранится в Redis. Изменить модели или скорость набора — редактируйте `config.toml` и передеплойте.
...
├── config.toml         # Модели и технические настройки
