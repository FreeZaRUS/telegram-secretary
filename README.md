...
Бот-секретарь для Telegram, который автоматически отвечает на сообщения через AI. Поддерживает обычные личные сообщения и **Telegram Business / автоматизацию чатов**. Имитирует живого человека: показывает индикатор набора текста с реалистичной задержкой.
